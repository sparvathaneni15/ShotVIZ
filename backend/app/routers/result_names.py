...
router = APIRouter(prefix="/result_names", tags=["result_names"])

@router.post("/", response_model=schemas.ResultNameBase, status_code=201)
def create_result_name(result: schemas.ResultNameCreate, db: Session = Depends(get_db)):
    return crud.create_result_name(db, result)

@router.get("/", response_model=List[schemas.ResultNameBase])
...
@router.get("/{result_id}", response_model=schemas.ResultNameBase)
