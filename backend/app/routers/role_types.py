...
router = APIRouter(prefix="/role_types", tags=["role_types"])

@router.post("/", response_model=schemas.RoleTypeBase, status_code=201)
def create_role_type(role: schemas.RoleTypeCreate, db: Session = Depends(get_db)):
    return crud.create_role_type(db, role)

@router.get("/", response_model=List[schemas.RoleTypeBase])
...
@router.get("/{role_id}", response_model=schemas.RoleTypeBase)
